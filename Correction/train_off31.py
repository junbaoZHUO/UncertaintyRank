...
parser.add_argument('--noisetype', default='corruption', type=str)
...
def train(train_loader, model, optimizer, epoch, update_num, M, V, TARGET, traindir):
    ...
    for i in range(train_len):
        ...
        no_swap_acc = 0
        swap_acc = 0
        SWAP_ACC = open("../SWAP_ACC_0.4.txt",'a')
        NO_SWAP_ACC = open("../NO_SWAP_ACC_0.4.txt",'a')
        for ii in range(int(ff.size(0))):
            TT[ii]= np.random.randint(0, _mu_.size(1)) 
            while TT[ii]==int(T[ii].item()) or TT[ii] ==int(PP[ii].item()):
                TT[ii]= np.random.randint(0, _mu_.size(1)) 
            if TT[ii].item()==int(c[index[ii].item()].split()[-1]):
                swap_acc+=1
            if T[ii].item()==int(c[index[ii].item()].split()[-1]):
                no_swap_acc+=1
        SWAP_ACC.write(str(float(swap_acc)/int(ff.size(0)))+'\n')
        NO_SWAP_ACC.write(str(float(no_swap_acc)/int(ff.size(0)))+'\n')
        ...
        lc = 0.5 * (nn.CrossEntropyLoss(reduce=False).cuda()(_mu_ ,T.long().cuda()).view(-1,1)  *torch.exp(0-_epsilon_)).mean() + _epsilon_.mean() * 0.5 \
           + torch.clamp(0.10 + fast_out1  - fast_out , 0, 1000).mean() \
