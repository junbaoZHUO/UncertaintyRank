...
def train(model_instance, train_source_clean_loader, train_source_noisy_loader, train_target_loader, test_target_loader, group_ratios, max_iter, optimizer, lr_scheduler, eval_interval):
    ...
    while True:
        for (datas_clean, datas_noisy, datat) in tqdm.tqdm(
            ...
            total_loss = train_batch(model_instance, inputs_source, labels_source, inputs_target, optimizer, iter_num, max_iter, inputs_source_noisy)
    ...
    print('finish train')
    return [loss, result]
def train_batch(model_instance, inputs_source, labels_source, inputs_target, optimizer, iter_num, max_iter, inputs_source_noisy):
    # inputs = torch.cat((inputs_source, inputs_target), dim=0)
    # optimizer.extrapolation()
    inputs = torch.cat((inputs_source, inputs_target, inputs_source_noisy), dim=0)
    total_loss = model_instance.get_loss(inputs, labels_source, len(inputs_source_noisy))
