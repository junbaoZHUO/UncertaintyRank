...
def my_forward(feat,fast_weights, C1):
    ...
    return torch.nn.LeakyReLU()(fast_out)#torch.clamp(fast_out, -1, 10)
...
def main():
    ...
    traindir2 = './office_list/'+args.traindata+'_list.txt'
    ...
    model = network.ResNetFc("ResNet50",use_bottleneck=False,new_cls=True, class_num=31).cuda()
    V = network.Classifier(2048, 512, cls=1).cuda()
    M = nn.Linear(2048,31).cuda()#network.Classifier(64, 64, cls=10).cuda()
    M.weight.data.normal_(0, 0.01)
    M.bias.data.fill_(0.0)
    V = nn.Linear(2048, 1).cuda()#network.Classifier(64, 64, cls=10).cuda()
    V.weight.data.normal_(0, 0.01)
    V.bias.data.fill_(0.0)
    BN = torch.nn.BatchNorm1d(31).cuda()
    ...
    TARGET = torch.zeros((len(open(traindir).readlines()), 31)).cuda()
...
def validate(val_loader, model, C1, V, epoch, loader, traindir, EEE, TARGET):
    ...
    nr = 0.2#float(traindir.split('.txt')[0].split('list_noisycorrupted_')[1])/2
    cr = 0.8#min(1-1.2*nr, 0.9*(1-nr))
    print(nr)
    print(cr)
    with torch.no_grad():
        cor = 0
