...
import torch
import sys
sys.path.insert(0, "/data1/junbao/RDA")
...
def train(model_instance, train_source_clean_loader, train_target_loader, test_target_loader, group_ratios, max_iter, optimizer, lr_scheduler, eval_interval):
    ...
    while True:
        for (datas_clean, datat) in tqdm.tqdm(
            ...
            total_loss = train_batch(model_instance, inputs_source, labels_source, inputs_target, optimizer, iter_num, max_iter, WEIGHT)
    ...
    print('finish train')
    return [loss, result]
def train_batch(model_instance, inputs_source, labels_source, inputs_target, optimizer, iter_num, max_iter, WEIGHT):
    # inputs = torch.cat((inputs_source, inputs_target), dim=0)
    # optimizer.extrapolation()
    inputs = torch.cat((inputs_source, inputs_target), dim=0)
    total_loss = model_instance.get_loss(inputs, labels_source, WEIGHT.cuda())
...
if __name__ == '__main__':
    from model.MDDU import MDDU as MDD
    from model.MDD3 import MDD
    from preprocess.data_provider_ab import load_images
    import pickle
    ...
    parser.add_argument('--noisy_rate', default=None, type=float,
                        help='noisy rate')
    parser.add_argument('--maxIter', default=40000, type=int,
                        help='max iteration')
    ...
    train_source_clean_loader = load_images(source_file, batch_size=32, is_cen=is_cen, split_noisy=True)
    ...
    import sys
    sys.path
    sys.path.append('/data1/junbao/RDA/optimex')
    from optimex import ExtraAdam
    optimizer = torch.optim.SGD(param_groups, **cfg.optim.params)
    # optimizer = ExtraAdam(param_groups, lr=0.00001, betas=(0.5, 0.9))
    ...
    lr_scheduler = INVScheduler(gamma=cfg.lr_scheduler.gamma,
                                ...
                                init_lr=cfg.init_lr)
    to_dump = train(model_instance, train_source_clean_loader, train_target_loader, test_target_loader, group_ratios, max_iter=args.maxIter, optimizer=optimizer, lr_scheduler=lr_scheduler, eval_interval=1000)
