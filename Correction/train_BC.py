...
parser.add_argument('-c', '--classes', default=257, type=int, metavar='N',
    ...
    for i in range(train_len):
        ...
        for ii in range(int(ff.size(0))):
            ...
            while TT[ii]==int(T[ii].item()) or TT[ii] ==int(PP[ii].item()):
                TT[ii]= np.random.randint(0, _mu_.size(0)) 
...
def validate(model, C1, V, epoch, loader, traindir, EEE, TARGET):
    ...
    c = open('BC_list/'+traindir.split('_')[1].split('/')[1]+'_list.txt').readlines()
