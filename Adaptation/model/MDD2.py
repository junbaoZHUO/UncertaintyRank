...
class MDD(object):
    ...
    def get_loss(self, inputs, labels_source, N_N, WEIGHT):
        ...
        classifier_loss_adv_src = (nn.CrossEntropyLoss(reduce=False)(outputs_adv.narrow(0, 0, labels_source.size(0)), target_adv_src).view(-1,1)*normalize_weight(0-WEIGHT)).mean()#.view(-1,1)*normalize_weight(torch.exp(0-U_s.detach()))).mean()
        ...
        self.iter_num += 1
        total_loss = classifier_loss + transfer_loss + 0.10*en_loss
