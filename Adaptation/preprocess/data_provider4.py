    ...
    if not is_train:
        ...
        if is_cen:
            ...
            transformer = transforms.Compose([ResizeImage(resize_size),
                  ...
                  normalize])
        if split_noisy:
            clean_images = ImageList(open(images_file_path.split('.t')[0]+'_true_pred.txt3').readlines(), transform=transformer)
            noisy_images = ImageList(open(images_file_path.split('.t')[0]+'_false_pred.txt1').readlines(), transform=transformer)
