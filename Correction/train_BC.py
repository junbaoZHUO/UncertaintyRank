...
parser.add_argument('--gpu', default='5', type=str, metavar='N',
                    help='visible gpu')
...
def main():
    ...
    model = network.ResNetFc("ResNet50",use_bottleneck=False,new_cls=True, class_num=257).cuda()
    V = network.Classifier(2048, 512, cls=1).cuda()
    M = nn.Linear(2048,257).cuda()#network.Classifier(64, 64, cls=10).cuda()
    M.weight.data.normal_(0, 0.01)
    M.bias.data.fill_(0.0)
    V = nn.Linear(2048, 1).cuda()#network.Classifier(64, 64, cls=10).cuda()
    V.weight.data.normal_(0, 0.01)
    V.bias.data.fill_(0.0)
    BN = torch.nn.BatchNorm1d(257).cuda()
    ...
    TARGET = torch.zeros((len(open(traindir).readlines()), 257)).cuda()
    ...
    for epoch in range(75):
        ...
        iii=train(source_loader, model, optimizer, epoch, iii, M, V, TARGET, UU, WW, )

        EEE=validate( model, M, V, epoch, source_loader2, traindir,EEE,  TARGET)
        traindir2 = traindir.split('.txt')[0]+'_WEIGHT.txt'+str(epoch)
        AA=open(traindir2, 'w')
        for eee in EEE:
            AA.write(str(eee)+'\n')
        AA.close()

def train(train_loader, model, optimizer, epoch, iii, M, V, TARGET, UU, WW):
    ...
    for i in range(train_len):#, (inputs, target) in enumerate(train_loader):
        ...
        lc += elr_reg -1.0*Entropy(torch.mean(F.softmax(M_,dim=1),dim=0).view(1,-1)).sum()
        optimizer.zero_grad()
...
def validate( model, C1, V, epoch, loader, traindir, EEE, TARGET):
    ...
    TT=[]
    EE=[]
    traindir2 = traindir.split('.txt')[0]+str(epoch)+'_Relabel.txt'
    a = open(traindir2,'w')
    traindir2 = traindir.split('.txt')[0]+str(epoch)+'_Relabel_0.6.txt'
    a1 = open(traindir2,'w')
    traindir2 = traindir.split('.txt')[0]+str(epoch)+'_Relabel_0.7.txt'
    a2 = open(traindir2,'w')
    traindir2 = traindir.split('.txt')[0]+str(epoch)+'_Relabel_0.8.txt'
    a3 = open(traindir2,'w')
    traindir2 = traindir.split('.txt')[0]+str(epoch)+'_left.txt'
    aa = open(traindir2,'w')
    traindir2 = traindir.split('.txt')[0]+str(epoch)+'_left_0.6.txt'
    aa1 = open(traindir2,'w')
    traindir2 = traindir.split('.txt')[0]+str(epoch)+'_left_0.7.txt'
    aa2 = open(traindir2,'w')
    traindir2 = traindir.split('.txt')[0]+str(epoch)+'_left_0.8.txt'
    ...
    with torch.no_grad():
        ...
        if epoch > -1:
            ...
            YES4=[0*iiii for iiii in range(len(EE))]
            for cc in range(257):
                CCC=[]
                TTT=[]
